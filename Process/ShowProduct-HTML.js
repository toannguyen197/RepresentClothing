...
function TangDan(){
    ...
    for(let i=0;i<priceElement.length;i++){
            ...
            if (parseFloat(priceElement[i].innerText.replace('đ','')) > parseFloat(priceElement[j].innerText.replace('đ',''))){
                var temp = priceElement[i].parentElement.innerHTML
                priceElement[i].parentElement.innerHTML = priceElement[j].parentElement.innerHTML;
                priceElement[j].parentElement.innerHTML = temp;
            }
            }
    }
}
function GiamDan(){   var priceElement=document.getElementsByClassName('money');
    for(let i=0;i<priceElement.length;i++){
            for ( var j = i + 1; j < priceElement.length; j++){
            //Bỏ chữ đ sau giá tiền
            if (parseFloat(priceElement[i].innerText.replace('đ','')) < parseFloat(priceElement[j].innerText.replace('đ',''))){
